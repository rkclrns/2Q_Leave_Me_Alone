...
public:
	...
	Vector2() : x(0.f), y(0.f) {}
	// 오버로딩된 생성자
	Vector2(float _x, float _y = 0.f) : x(_x), y(_y) {}
	Vector2(int _x, int _y = 0) : x((float)_x), y((float)_y) {}
	Vector2(long _x, long _y = 0) : x((float)_x), y((float)_y) {}
	...
	float Distance(Vector2 _other)
	{
		float xDis = abs(x - _other.x);
		float yDis = abs(y - _other.y);
		return (float)sqrt((xDis * xDis) + (yDis * yDis));
	}

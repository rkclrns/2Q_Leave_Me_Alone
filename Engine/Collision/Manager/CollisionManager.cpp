	...
	for (Collider* _leftCollider : _leftcolliders)
	{
		for (Collider* _rightCollider : _rightcolliders)
		{
			...
			if (CheckCollision(_leftCollider, _rightCollider))
			{
				...
				if (check == false)
				{
					_leftCollider->OnCollisionEnter(_leftCollider, _rightCollider);
					_rightCollider->OnCollisionEnter(_rightCollider, _leftCollider);
					SetCollisionID(_leftCollider, _rightCollider, true);
				}
				// 이미 충돌중이었으면 Stay
				else if (check == true)
				{
					_leftCollider->OnCollisionStay(_leftCollider, _rightCollider);
					_rightCollider->OnCollisionStay(_rightCollider, _leftCollider);
				}
			}
			else
			{
				...
				if (check == true)
				{
					_leftCollider->OnCollisionExit(_leftCollider, _rightCollider);
					_rightCollider->OnCollisionExit(_rightCollider, _leftCollider);
